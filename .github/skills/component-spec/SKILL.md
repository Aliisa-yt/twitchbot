...
- コンポーネントは `ComponentBase` を継承する（定義: `src/core/components/base.py`）。
...
- `component_registry` の型は `dict[str, ComponentDescriptor]`。
  ...
  - `is_removable=True` のコンポーネントは `src/core/components/removable/` 配下に定義されたコンポーネントで、Bot運転中に動的にアタッチ/デタッチできる。
