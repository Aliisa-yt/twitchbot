...
description: core/trans/engines の翻訳エンジン契約、認証分岐、例外正規化、クォータ仕様ガイド
...
この文書は `core/trans/engines/` 配下の翻訳エンジンクラスを変更・追加する際の実装ガイドです。
...
### 3.1 `DeeplTranslation` (`core/trans/engines/trans_deepl.py`)
...
### 3.2 `GoogleTranslation` (`core/trans/engines/trans_google.py`)
...
### 3.3 `GoogleCloudTranslation` (`core/trans/engines/trans_google_cloud.py`)
