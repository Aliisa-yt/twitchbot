...
description: core/tts/engines のTTSエンジン契約、合成パラメータ仕様、例外設計ガイド
...
この文書は `core/tts/engines/` 配下のTTSエンジンクラスを変更・追加する際の実装ガイドです。
...
### 3.1 `VVCore` (`core/tts/engines/vv_core.py`)
...
### 3.2 `VoiceVox` (`core/tts/engines/voicevox.py`)
...
### 3.3 `CoeiroInk` (`core/tts/engines/coeiroink.py`)
...
### 3.4 `CoeiroInk2` (`core/tts/engines/coeiroink_v2.py`)
...
### 3.6 `BouyomiChanSocket` (`core/tts/engines/bouyomichan.py`)
...
### 3.7 `GoogleText2Speech` (`core/tts/engines/g_tts.py`)
