...
| フロー制御 | `TokenManager` | `core/token_manager.py` | OAuth フロー実行・トークン検証・キャッシュ確認・トークン形式変換 |
| Bot 統合 | `Bot.event_token_refreshed` / `Bot.load_tokens` | `core/bot.py` | TwitchIO へのトークン登録・起動時読み込み・リフレッシュ時 DB 更新 |
