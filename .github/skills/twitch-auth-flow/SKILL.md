...
### 4.5 トークン所有者確認 (`_validate_access_token_user_id`)
...
### 4.6 キャッシュ検証とキャッシュ破棄ロジック
- キャッシュが存在し `access_token` がある場合 → `_validate_access_token_user_id` で所有者を確認。
- `token_user_id != self.bot_id` の場合（例：owner アカウントで誤認証）→ キャッシュを破棄して再認証。
- バリデーション自体が失敗した場合（トークン期限切れ含む）→ 同様にキャッシュを破棄して再認証。
- **これが「owner アカウントで誤認証した場合の自動修復」の仕組みである。**
...
| キャッシュトークン検証失敗 | ログ warning + 再認証 | 自動復旧（例外は伝播しない） |
...
- DB に保存されたトークンの `user_id` が Bot ID と一致しない（Owner アカウントで誤認証した）。

**解決策**:
1. `tokens.db` を削除して `setup_tokens.py` を再実行する。
2. Bot アカウントでログインする。

### 11.3 ローカルサーバー起動失敗（ポート 80 が使用中）
...
- `start_authorization_flow`:
  ...
  - キャッシュトークンが Bot ID と不一致の場合はキャッシュを破棄して再認証すること。
  - キャッシュトークン検証が失敗した場合は再認証すること（例外は伝播しないこと）。
