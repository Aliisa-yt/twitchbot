...
def check_python_version() -> None:
    # Must be run on version 3.14 or later to use features implemented in Python 3.14.
    if sys.version_info < (3, 14):
        msg = "Python 3.14 or later is required"
...
def sig_handler(signum: int | signal.Signals, frame: FrameType | None) -> None:
    _ = signum, frame
...
class SignalHandler:
    def __init__(self, sig: signal._SIGNUM, handler: signal._HANDLER) -> None:  # pyright: ignore[reportPrivateUsage]
        self.sig: signal._SIGNUM = sig  # pyright: ignore[reportPrivateUsage]
        self.handler: signal._HANDLER = handler  # pyright: ignore[reportPrivateUsage]
        self.original_handler: signal._HANDLER | None = None  # pyright: ignore[reportPrivateUsage]
